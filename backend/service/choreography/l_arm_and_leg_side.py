...
def l_arm_and_leg_side(compare_manager, choreography_manager, index):
    sound_duration = 1
    ...
    dp_sum = 0
    for i in range(8):
        reference_pos = compare_manager.l_arm_and_leg_side[i][index-size][0:3]
        for j in range(size):
            euclid = euclidean(compare_manager.l_arm_and_leg_side[i][index-size+j][0:3] - reference_pos, choreography_manager.l_arm_and_leg_side[i][j][0:3])
            cos_sim = cosine(compare_manager.l_arm_and_leg_side[i][index-size+j][3:6], choreography_manager.l_arm_and_leg_side[i][j][3:6])
